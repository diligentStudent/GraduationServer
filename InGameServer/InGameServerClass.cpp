#include "pch.h"
#include "InGameServerClass.h"

InGameServerClass::InGameServerClass()
{
}
...
HRESULT InGameServerClass::init()
{
	xml_t config;
...
void InGameServerClass::release()
{
	TASKMANAGER.release();
...
void InGameServerClass::serverProcess()
{
	std::shared_ptr<Server> server(new IOCPServer(new InGameProcess()));
...
void InGameServerClass::run()
{
	std::shared_ptr<Thread> serverThread(new Thread(new thread_t(&InGameServerClass::serverProcess, this), L"Server"));
}
