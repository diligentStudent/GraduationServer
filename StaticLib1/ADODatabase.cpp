...
{
	std::array<WCHAR, SIZE_128> buffer;
...
bool ADODatabase::connect(const WCHAR *serverName, const WCHAR *dbName, const WCHAR *id, const WCHAR *password)
{
	...
	{
		std::array<WCHAR, SIZE_64> mssqlName;
...
void ADODatabase::execute()
{
	...
	try
	{
		...
		_variant_t resultVal;//컴객체에서 사용하는 auto(모든자료형을 받을수 있는 변수)

		//record에는 믿의 쿼리를 통해 테이블이 들어가게 된다
		switch (statement->type()) 
		{
		case QUERY_NOT_RETURN://리턴이 필요없어 기다리지 않는 쿼리
			record = command->Execute(&resultVal, NULL, ADODB::adCmdText | ADODB::adExecuteNoRecords);
			break;
		case QUERY_WAIT_RETURN://리턴을 무조건 받아 기다리는 쿼리
			record = command->Execute(&resultVal, NULL, ADODB::adCmdText | ADODB::adExecuteRecord);
			break;
		case QUERY_CALL_BACK://기다리진않지만 후처리는 해주는 쿼리
